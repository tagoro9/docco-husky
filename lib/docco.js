...
(function() {
  ...
  highlight = function(source, sections, callback) {
    var code, i, language, section, _i, _len;
    language = get_language(source);
  ...
  write_file = function(dest, contents) {
    ...
    return fs.stat(target_dir, function(err, stats) {
      ...
      if (err) {
        return exec("mkdir " + (target_dir.replace(/\//g, "\\")), function(err) {
          if (err && err.code !== 1) {
            throw err;
          }
          return write_func();
  ...
  ensure_directory = function(dir, callback) {
    return exec("mkdir " + (dir.replace("/", "\\")), function() {
      return callback();
    });
  };
